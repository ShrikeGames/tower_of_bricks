...
# Original Source: https://github.com/Ev01/PlatformerController2D/blob/main/platformer_controller/platformer_controller.gd
# 2024-08-17

@export var head_brick_node:Node2D
...
var achievement_double_jump_count:int = 0
var achievement_dance_history:Array[int] = []
var achievment_time_since_last_moved:float = 0
...
func _physics_process(delta):
	...
	if not is_feet_on_ground():
		velocity += Global.get_wind_speed(self.position) * 60 *delta
	var mouse_position:Vector2 = camera.get_global_mouse_position()
	if mouse_position.x > self.global_position.x:
		sprite.set_frame_and_progress(0, 0)
		achievement_dance_history.append(1)
	elif mouse_position.x < self.global_position.x:
		sprite.set_frame_and_progress(1, 0)
		achievement_dance_history.append(-1)
	achievement_dance_history = achievement_dance_history.slice(max(0, len(achievement_dance_history)-60), min(60, len(achievement_dance_history)))
...
func can_double_jump():
	...
	if jumps_left > 0 and not is_feet_on_ground() and coyote_timer.is_stopped():
		achievement_double_jump_count += 1
		return true
...
# Calculates the desired gravity from jump height and jump duration.
...
# Calculates the gravity when the key is released based off the minimum jump height and jump velocity.
...
# Returns a value for friction that will hit the max speed after 90% of time_to_max seconds.
...
func remove_head_brick():
	for brick_object in head_brick_node.get_children():
		brick_object.queue_free()
		
func add_head_brick(brick:Brick):
	remove_head_brick()
	brick.position = Vector2(0,0)
	brick.is_ghost = true
	brick.freeze = true
	brick.collision_layer = 2
	brick.collision_mask = 2
	brick.apply_scale(Vector2(0.3,0.3))
	head_brick_node.add_child(brick)
